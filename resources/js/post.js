...
$('.post_box_content .post_content').each(function () {
    if ($(this).find('.post_card').length > 0) {
        ...
        let post_cart = $(this).find('.post_card');
        let content = post_cart.attr('value').replaceAll('\\n', '\n');
        let val = post_cart.attr('lang');

        view[post_cart.attr('id')] = new EditorView({
...
$('.run_code').click(function () {
    ...
    let code_editer = $(this).parent().parent().find('.post_card');
